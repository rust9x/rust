...
pub(crate) unsafe fn get_file_information_by_handle_ex(
    ...
    unsafe {
        ...
        let (class, min_buffer_size, use_directory_api_info): (
            ...
            //
            // rust9x NOTE: these two (FileIdBothDirectoryInfo, FileIdBothDirectoryRestartInfo) are
            // replaced with FileFullDirectoryInfo/FileFullDirectoryRestartInfo below, as the Full
            // versions are available on older Windows versions.
            //
        ...
        if status == STATUS_PENDING {
            status = NtWaitForSingleObject(file_handle, false, core::ptr::null_mut());
        }
